...
namespace ncore
{
    namespace npacket
    {
        ...
        namespace ntype
        {
            ...
            const type_t Temperature = 0x01 | (nfieldtype::TypeS8 << 8);   // T    (s8, °C)
            const type_t Humidity    = 0x02 | (nfieldtype::TypeU8 << 8);   // H    (u8, %)
            const type_t Pressure    = 0x03 | (nfieldtype::TypeU16 << 8);  // HPA  (u16, hPa)
            const type_t Light       = 0x04 | (nfieldtype::TypeU16 << 8);  // LUX  (u16, lux)
            const type_t CO2         = 0x05 | (nfieldtype::TypeU16 << 8);  // CO2  (u16, ppm)
            const type_t VOC         = 0x06 | (nfieldtype::TypeU16 << 8);  // VOC  (u16, ppm)
            const type_t PM1_0       = 0x07 | (nfieldtype::TypeU16 << 8);  // PM1  (u16, µg/m3)
            const type_t PM2_5       = 0x08 | (nfieldtype::TypeU16 << 8);  // PM2  (u16, µg/m3)
            const type_t PM10        = 0x09 | (nfieldtype::TypeU16 << 8);  // PMA  (u16, µg/m3)
            const type_t Noise       = 0x0A | (nfieldtype::TypeU8 << 8);   // DB   (u8, dB)
            const type_t UV          = 0x0B | (nfieldtype::TypeU8 << 8);   // UV   (u8, index)
            const type_t CO          = 0x0C | (nfieldtype::TypeU8 << 8);   // CO   (u16, ppm/10)
            const type_t Vibration   = 0x0D | (nfieldtype::TypeU8 << 8);   // V    (u8, 0=none, 1=low, 2=medium, 3=high)
            const type_t State       = 0x0E | (nfieldtype::TypeU16 << 8);  // S    (u16, state)
            const type_t Presence1   = 0x10 | (nfieldtype::TypeU8 << 8);   // P1   (u8, 0=none, 1=trigger up/down, 2=presence)
            const type_t Presence2   = 0x11 | (nfieldtype::TypeU8 << 8);   // P2   (u8, 0=none, 1=trigger up/down, 2=presence)
            const type_t Presence3   = 0x12 | (nfieldtype::TypeU8 << 8);   // P3   (u8, 0=none, 1=trigger up/down, 2=presence)
            const type_t Distance1   = 0x13 | (nfieldtype::TypeU16 << 8);  // D1   (u16, cm)
            const type_t Distance2   = 0x14 | (nfieldtype::TypeU16 << 8);  // D2   (u16, cm)
            const type_t Distance3   = 0x15 | (nfieldtype::TypeU16 << 8);  // D3   (u16, cm)
