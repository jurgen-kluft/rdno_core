...
UNITTEST_SUITE_BEGIN(config)
{
    UNITTEST_FIXTURE(general)
    {
        ...
        s16 KeyToIndex(str_t const& str)
        {
            ...
            s32 value = 0;
            if (from_str(str, &value, 10) && value >= 0 && value < nconfig::PARAM_ID_MAX_COUNT)
            {
        ...
        UNITTEST_TEST(parse_message_1)
        {
            ...
            nconfig::set_int(&config, nconfig::PARAM_ID_REMOTE_PORT, 12);                  // remote_port
            ...
            CHECK_TRUE(config.m_param_types[nconfig::PARAM_ID_REMOTE_PORT] == nconfig::PARAM_TYPE_S32);

            s32 remote_port;
            CHECK_TRUE(nconfig::get_int(&config, nconfig::PARAM_ID_REMOTE_PORT, remote_port));
